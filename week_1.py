...
def generate_noisy_linear(num_samples, weights, sigma, limits, rng):
    ...
    # TODO: implement this
    return utils.random_sample(lambda x: utils.affine(x, weights), len(weights) - 1, num_samples, limits,
                                rng, sigma)
...
def hyperplane(X, bound):
    y = utils.affine(X, bound)
...
def generate_linearly_separable(num_samples, weights, limits, rng):
    ...
    """
    # TODO: implement this
    return utils.random_sample(lambda x: hyperplane(x, weights), count=len(weights) - 1, 
